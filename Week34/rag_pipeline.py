...
A production-style Retrieval-Augmented Generation pipeline demonstrating:
    ...
    4. Vector storage             (ChromaDB PersistentClient)
    5. Semantic retrieval         (cosine similarity, top-k)
    6. Answer generation          (OpenAI gpt-4o-mini, grounded)

Demo corpus:
    "Attention Is All You Need" (Vaswani et al., 2017) — open-access arXiv paper.
    Google has granted permission to reproduce its tables and figures for
    journalistic / scholarly works, making it a clean public-domain demo source.

Usage:
    export OPENAI_API_KEY=sk-...
    python -m src.rag_pipeline ingest        # one-time: download + index
    python -m src.rag_pipeline ask "What is multi-head attention?"
    python -m src.rag_pipeline ask "How is the model trained?"  --k 6
    python -m src.rag_pipeline chat          # interactive REPL
...
# OpenAI models. Embedding model output dim = 1536 for text-embedding-3-small.
EMBED_MODEL = "text-embedding-3-small"
CHAT_MODEL = "gpt-4o-mini"

# Chunking: words (cheap & deterministic). For production, prefer a tokenizer
# (tiktoken) so chunk size matches embedding model context exactly.
CHUNK_WORDS = 220        # ≈ 280–300 tokens — well within 8192 input limit
CHUNK_OVERLAP_WORDS = 40 # 18% overlap preserves context across boundaries
...
class OpenAIEmbedder:
    ...
    - Batches inputs (OpenAI accepts up to ~2048 inputs per call, but we cap
      smaller to keep individual requests fast and cheap to retry).
    - Retries on transient errors with exponential backoff.
    """

    def __init__(self, client: OpenAI, model: str = EMBED_MODEL, batch_size: int = 64):
        self.client = client
        self.model = model
    ...
    def _embed_with_retry(self, batch: list[str], max_attempts: int = 5) -> list[list[float]]:
        delay = 1.0
        for attempt in range(1, max_attempts + 1):
            try:
                resp = self.client.embeddings.create(model=self.model, input=batch)
                # Response order matches input order — preserve it.
...
def get_chroma_collection(reset: bool = False):
    ...
    PersistentClient writes to disk (SQLite + parquet under CHROMA_DIR), so
    re-running the script keeps state across invocations.

    We deliberately do NOT pass an embedding_function to Chroma, because we
    embed manually with OpenAI and pass vectors via `embeddings=...`. This
    keeps the embedding step explicit and observable in logs, and avoids
    Chroma's auto-embedding magic which can hide retries / token usage.
    """
    ...
    # hnsw:space=cosine matches OpenAI embeddings, which are L2-normalized.
...
def index_chunks(collection, chunks: list[Chunk], embedder: OpenAIEmbedder) -> None:
...
def retrieve(
    ...
    """Embed the query and pull the top-k most similar chunks."""
...
def ask_pipeline(question: str, top_k: int = DEFAULT_TOP_K, show_sources: bool = True) -> str:
    ...
    if collection.count() == 0:
        raise RuntimeError(
            "Collection is empty. Run `python -m src.rag_pipeline ingest` first."
        )
...
def chat_repl() -> None:
    ...
    print("\nRAG REPL — ask questions about the Transformer paper.")
...
def main() -> None:
    p = argparse.ArgumentParser(
        prog="rag_pipeline",
        description="End-to-end RAG demo: PyMuPDF + Chroma + OpenAI",
    )
